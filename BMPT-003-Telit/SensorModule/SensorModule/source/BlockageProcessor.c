...
#define NOISE_NO_FLOW_THRESHOLD 10//1       // Noise threshold for sensor-wide zero-stuffing  
...
WORD sensorCalVals[NUM_SENSOR_ROWS][NUM_SENSORS_PER_ROW];   // Calibrated DAC settings for achieving a 2.5V sensor output
WORD sensorBaseline[NUM_SENSOR_ROWS][NUM_SENSORS_PER_ROW];  // Output signal recorded with calibrated DAC setting. Should be 2.5V +/- cal window.
...
static BYTE disableSensorData = 0;
...
void processBlockageData(void)
{
    ...
    if(!disableSensorData)
    {
        sendCANSensorDataMsg(rowLevel); 
...
BYTE getMaxSensorDataIndex(BYTE rowNumber)
{
    ...
    {
        //Check for magnet in each row. Must see minimum signal strength with minimum flow height (index 2 or greater) to activate.
            
        if((noMagnetRowMask & (0x01 << rowNumber)) && (maxValue > INITIAL_MAGNET_DETECT_THRESHOLD) && (index > 1))
        {
            noMagPersistenceCount[rowNumber]++;
                
            if(noMagPersistenceCount[rowNumber] > 2) // Must see signal for at least 3 consecutive samples
            {
