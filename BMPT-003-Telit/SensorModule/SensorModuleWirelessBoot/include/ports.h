...
//Input/Output PB1: PCINT1/MOSI/PSCOUT2B                (LED_ROW4) // Rev B LED 1
//Input/Output PB2: ADC5/INT1/ACMPN0/PCINT2             (AIN5)
//Input/Output PB3: AMP0-/PCINT3                        (LED_ROW2) // Rev B LED 4
//Input/Output PB4: AMP0+/PCINT4                        (LED_ROW1) // Rev B LED 3
...
#ifdef _REV_B_HW
    #define TOGGLE_LED	    PORTB ^= (_BV(PB1))
    #define TURN_LED_ON     PORTB |= (_BV(PB1))
    #define TURN_LED_OFF    PORTB &= ~(_BV(PB1))
#else
...
#ifdef _REV_B_HW
    #define TOGGLE_ROW2_LED     PORTC ^= (_BV(PC6))
    #define TURN_ON_ROW2_LED    PORTC |= _BV(PC6)
#else
    #define TOGGLE_ROW2_LED     PORTB ^= (_BV(PB3))
    #define TURN_ON_ROW2_LED    PORTB |= _BV(PB3)
#endif

#ifdef _REV_B_HW
    #define TOGGLE_ROW4_LED     PORTB ^= (_BV(PB3))
    #define TURN_OFF_ROW4_LED   PORTB &= ~(_BV(PB3))
    #define TURN_ON_ROW4_LED	PORTB |= _BV(PB3)
#else
    #define TOGGLE_ROW4_LED     PORTB ^= (_BV(PB1))
    #define TURN_OFF_ROW4_LED   PORTB &= ~(_BV(PB1))
    #define TURN_ON_ROW4_LED	PORTB |= _BV(PB1)
#endif
...
//Input/Output PC1: PCINT9/PSCIN1/OC1B/SS_A     (WiFi Level shifter enables)
...
//Input/Output PC6: ADC10/ACMP1/PCINT14         (LED_ROW3) // Rev B LED 2
...
#define ENABLE_UART_SHIFTERS	PORTC &= ~(_BV(PC1))

#ifdef _REV_B_HW
    #define TOGGLE_ROW3_LED PORTB ^= (_BV(PB4))
#else
    #define TOGGLE_ROW3_LED PORTC ^= (_BV(PC6))
#endif
...
//Input/Output PD2: PCINT18/PSCIN2/OC1A/MISO_A          (WiFi RTS)
//Input/Output PD3: PCINT19/TXD/TXLIN/OC0A/SS/MOSI_A    (MCU TXD, GS RXD)
//Input/Output PD4: PCINT20/ADC1/RXD/RXLIN/ICP1A/SCK_A  (MCU RXD, GS TXD)
//Input/Output PD5: ADC2/ACMP2/PCINT21                  (AIN2)
//Input/Output PD6: ADC3/ACMPN2/INT0/PCINT22            (AIN3)
//Input/Output PD7: ACMP0/PCINT23                       (WiFi CTS) 

#define CONFIG_PORTD_DDR DDRD |= (_BV(PD0) | _BV(PD1) | _BV(PD3) | _BV(PD4) | _BV(PD7))


//Port D pin access macros
#define SET_WIFI_CTS_LOW		PORTD &= ~(_BV(PD7))
#define SET_WIFI_CTS_HIGH		PORTD |= _BV(PD7)
#define IS_WIFI_RTS_LOW			((PORTD & _BV(PD2)) == 0)
#define IS_WIFI_RTS_HIGH		((PORTD & _BV(PD2)) != 0)
#define SET_WIFI_RESET_HIGH     PORTD &= ~(_BV(PD1)) // logic inverted by hardware
#define SET_WIFI_RESET_LOW      PORTD |= _BV(PD1)
