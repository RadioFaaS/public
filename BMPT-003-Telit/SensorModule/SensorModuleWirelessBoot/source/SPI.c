...
void addBytesToWifiTxBuffer(BYTE * data, BYTE size)
{
    ...
    for(i = 0; i < size; i++)  //Removed for debug -ssb
    {
		...
		/*for(i = 0; i < 1; i++)
        {
            wifiTxBuffer[wifiTxBufferTail++] = 'A';
            if(wifiTxBufferTail >= WIFI_TX_BUFFER_SIZE)
            {
                wifiTxBufferTail = 0;
            }
            wifiTxBuffer[wifiTxBufferTail++] = 'T';
            if(wifiTxBufferTail >= WIFI_TX_BUFFER_SIZE)
            {
                wifiTxBufferTail = 0;
            }     
            wifiTxBuffer[wifiTxBufferTail++] = 'E';
            if(wifiTxBufferTail >= WIFI_TX_BUFFER_SIZE)
            {
                wifiTxBufferTail = 0;
            }
            wifiTxBuffer[wifiTxBufferTail++] = '1';
            if(wifiTxBufferTail >= WIFI_TX_BUFFER_SIZE)
            {
                wifiTxBufferTail = 0;
            }
            wifiTxBuffer[wifiTxBufferTail++] = '\r';
            if(wifiTxBufferTail >= WIFI_TX_BUFFER_SIZE)
            {
                wifiTxBufferTail = 0;
            }
            wifiTxBuffer[wifiTxBufferTail++] = '\n';
            if(wifiTxBufferTail >= WIFI_TX_BUFFER_SIZE)
            {
                wifiTxBufferTail = 0;
            }                   
